...
public static partial class CategoriesEndpoints
{
    ...
    {
        app.MapPost("", async (ILogger<Logging> logger, ICategoryUseCases categoryUseCases, CategoryInput category, CancellationToken cancellation) =>
        {
            return await Create(logger, categoryUseCases, category, cancellation);
        })
        .Accepts<CategoryInput>(DefaultContentType)
